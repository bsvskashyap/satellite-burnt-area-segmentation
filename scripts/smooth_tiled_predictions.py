...
def _spline_window(window_size, power=2):
    ...
    Args:
        ...
        power (int): Power parameter for the spline window.

    Returns:
        numpy.ndarray: 1D window function of shape (window_size,).
    """
    intersection = int(window_size / 4)
    wind_outer = (abs(2 * (scipy.signal.triang(window_size))) ** power) / 2
    wind_outer[intersection:-intersection] = 0
...
def _window_2D(window_size, power=2):
    ...
    Returns:
        numpy.ndarray: 2D window function of shape (window_size, window_size).
    """
    # Memoization
    ...
    # Create the 1D window function using _spline_window
    window_1D = _spline_window(window_size, power)

    # Convert the 1D window to a 2D window
    window_2D = np.outer(window_1D, window_1D)

    # Reshape the 2D window to add a single channel dimension
    window_2D = window_2D[:, :, np.newaxis]

    # Cache the 2D window
    cached_2d_windows[key] = window_2D

    # Plot the 2D window if PLOT_PROGRESS is True
    if PLOT_PROGRESS:
        plt.imshow(window_2D[:, :, 0], cmap="viridis")
...
def predict_img_with_smooth_windowing(input_img, window_size, subdivisions, nb_classes, pred_func):
    ...
    print("Input image shape before prediction:", input_img.shape)
    if PLOT_PROGRESS:
        plt.imshow(prd)
